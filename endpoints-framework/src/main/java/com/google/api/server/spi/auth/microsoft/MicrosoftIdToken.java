...
public class MicrosoftIdToken extends JsonWebSignature {
	...
	public boolean verifyIssuer(String issuer) {
		return issuer.equals(getPayload().getIssuer());
	}
	
	public boolean verifyVersion() {
		return SupportedVersion.isSupported(getPayload().getVersion());
	}
	...
	public final boolean verifyExpirationTime(long currentTimeMillis, long acceptableTimeSkewSeconds) {
		return currentTimeMillis <= (getPayload().getExpirationTimeSeconds() + acceptableTimeSkewSeconds) * 1000;
	}
	
	public final boolean verifyIssuedAtTime(long currentTimeMillis, long acceptableTimeSkewSeconds) {
		return currentTimeMillis >= (getPayload().getIssuedAtTimeSeconds() - acceptableTimeSkewSeconds) * 1000;
	}
